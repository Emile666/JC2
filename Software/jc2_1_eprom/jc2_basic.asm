...
TK_NORMAL	= TK_CLS+1		; NORMAL token
TK_INVERSE	= TK_NORMAL+1		; INVERSE token
TK_FLASH	= TK_INVERSE+1		; FLASH token
TK_LOCATE	= TK_FLASH+1		; LOCATE token	
TK_INNUM	= TK_LOCATE+1		; IN# token
TK_PRNUM	= TK_INNUM+1		; PR# token
TK_PORTIO	= TK_PRNUM+1		; PORTIO token
TK_PORTOUT	= TK_PORTIO+1		; PORTOUT token
TK_SCREEN	= TK_PORTOUT+1		; SCREEN token ###
TK_PIXEL	= TK_SCREEN+1		; PIXEL token ###
TK_LINE		= TK_PIXEL+1		; LINE token ###
TK_OVAL		= TK_LINE+1		; OVAL token ###
...
TK_MOD		= TK_DIV+1		; MOD token ###
TK_POWER	= TK_MOD+1		; ^ token
...
; #########################	
	JSR	CLEAR_BASE			; clear first two bytes of memory to avoid crash
; #########################	
...
LAB_SWAP
	...
	BNE	SwapErr			; exit if not both the same type  (changed 2022/05/16 by Joerg Walke)
...
; *****************************************
; ** New Commands For Junior Computer 2 ***
; *****************************************
LAB_MOD
	RTS
;	LDX	#$04
...
LAB_GETSTRPARM
	STA	Temp1
	JSR	LAB_EVEX		; evaluate expression
	LDA	Dtypef			; is it a string expression
	BNE	LAB_GETSTR0		; yes, get string				
	JSR	LAB_1ABC		; no, throw type mismatch error
LAB_GETSTR0
	LDY	#$00			; set string index to 0
LAB_GETSTR1
	LDA	(ssptr_l),Y		; load a string char
	BEQ	LAB_GETSTR2		; is it a NULL? yes, exit
	CMP	#'"'			; is it a '"'?
	BEQ	LAB_TERM		; yes, set string termination
	INY				; increment string index
	BNE	LAB_GETSTR1		; and repeat loop
LAB_TERM
	LDA 	#$00			; store termination char
	STA 	(ssptr_l),Y		; at the end of the string
LAB_GETSTR2
	LDX	ssptr_l			; load string pointer low byte into X
	LDY	ssptr_h			; load string pointer high byte into X
	LDA	Temp1
	RTS

; Get Device Parameters *******************
LAB_GETDEVPARM
	JSR	LAB_GTBY		; scan for byte value
	TXA				; and set A to result
	BEQ	LAB_DEV0		; device is 0, no more parameters needed
	STA	Temp1			; save device number
	JSR	LAB_1C01		; check for ','
	LDA	Temp1
	RTS
LAB_DEV0
	JMP	LAB_SCCA		; check if more token, if so throw syntax error

; Get Two Byte Parameters *****************
LAB_GET2BYTEPARMS
	;JSR	LAB_EVBY		; evaluate byte expression, result in X
	JSR	LAB_GTBY		; get first byte parameter into X
	STX	Temp1			; and store it in temp var
	JSR	LAB_1C01		; check for ',' else syntax error
	JSR	LAB_GTBY		; get second byte parameter into X
	TXA
	TAY				; and transfer it into Y
	LDX	Temp1			; restore first parameter into X
	RTS

; get device number parameter *************
LAB_GETDEVID
	JSR	LAB_GTBY		; get byte parameter into X
	TXA				; and transfer it into A
...
CALL_LOAD
	LDA	#CMD_LOAD
	JSR	CALL_CMD		; call load command
	BCC	CLEAR_BASE		; load interrupted, exit
	LDX	Ram_base-1
	STX	Svarl
	LDX	Ram_base
	STX	Svarh
...
; perform SAVE ****************************
LAB_SAVE
	BNE	SAVE_DEV		; if no following token use device 0
	LDA	#$00			; set device id = 0
	BEQ	SET_SAVEADR
SAVE_DEV
	JSR	LAB_GETDEVPARM		; get device id
...
; perform BEEP ****************************
LAB_BEEP
	BEQ 	LAB_JC001		; if no following token go do 'BEEP'
	RTS
...
; perform PLIST ***************************
; #### TODO: save current output device and restore pointer after list command !!!!!!!
LAB_PLIST
    	PHA				; save token
...
LAB_DELAY
	JSR	LAB_EVNM		; evaluate expression and check is numeric,
					; else do type mismatch
	JSR	LAB_EVPI		; evaluate positive integer expression
	LDA	IOBASE+1
	BEQ	LAB_DELAY_OLD		; check if IO is available (needed for RAM based EhBasic only)
	LDY	#VIA_IER		; select interrupt enable register
	LDA	#$7F
	STA	(IOBASE),Y		; disable all VIA2 interrupts
LAB_DELAY_LOOP
	LDY	#VIA_T2CL		; select Timer2 lower byte register
	LDA	#$E8			; set Timer2
	STA	(IOBASE),Y		; store timer low value
	LDA	#$03
	INY				; select Timer2 higher byte register
	STA	(IOBASE),Y		; store timer high value and start timer
	LDY	#VIA_IFR		; select interrupt flag register
	LDA	FAC1_2
	BNE	LAB_WAIT_TIMER
	LDA	FAC1_3
	BEQ	LAB_DELAY_END
...
LAB_CTBL
	...
	.word	LAB_CLS-1		; CLS		new command (Junior Computer 2)
	.word	LAB_NORMAL-1		; NORMAL	new command (Junior Computer 2)
	.word	LAB_INVERSE-1		; INVERSE	new command (Junior Computer 2)
	.word	LAB_FLASH-1		; FLASH		new command (Junior Computer 2)
	.word	LAB_LOCATE-1		; LOCATE	new command (Junior Computer 2)
	.word	LAB_INNUM-1		; IN#		new command (Junior Computer 2)
	.word	LAB_PRNUM-1		; PR#		new command (Junior Computer 2)
	.word	LAB_PORTIO-1		; PORTIO	new command (Junior Computer 2)
	.word	LAB_PORTOUT-1		; PORTOUT	new command (Junior Computer 2)
	.word	LAB_SCREEN-1		; SCREEN	new command (Junior Computer 2)	
	.word	LAB_PIXEL-1		; PIXEL		new command (Junior Computer 2)
	.word	LAB_LINE-1		; LINE		new command (Junior Computer 2)	
	.word	LAB_OVAL-1		; OVAL		new command (Junior Computer 2)	
...
LAB_OPPT
	...
	.byte	$7B			; /
	.word	LAB_DIVIDE-1
	.byte	$7B			; MOD		new operator (Junior Computer 2)
	.word	LAB_MOD-1
	.byte	$7F			; ^
	.word	LAB_POWER-1
	.byte	$50			; AND
...
LBB_OR
	.byte	'R',TK_OR		; OR
LBB_OVAL
	.byte	'VAL',TK_OVAL		; OVAL
	.byte	$00
TAB_ASCP
LBB_PEEK
	.byte	'EEK(',TK_PEEK		; PEEK(
LBB_PIXEL
	.byte	'IXEL',TK_PIXEL		; PIXEL
LBB_PI
	.byte	'I',TK_PI		; PI
LBB_PLIST
    .byte   	'LIST',TK_PLIST 	; PLIST
LBB_POKE
	.byte	'OKE',TK_POKE		; POKE
LBB_PORTIO
	.byte	'ORTIO',TK_PORTIO	; PORTIO
LBB_PORTIN
	.byte	'ORT(',TK_PORTIN	; PORT IN
LBB_PORTOUT
	.byte	'ORT',TK_PORTOUT	; PORT OUT
LBB_POS
	.byte	'OS(',TK_POS		; POS(
LBB_PRNUM
	.byte	'R#',TK_PRNUM		; PR#
...
LAB_KEYT
	...
	.byte	6,'P'
	.word	LBB_PORTIO		; PORTIO
	.byte	4,'P'
	.word	LBB_PORTOUT		; PORTOUT
	.byte	6,'S'
	.word	LBB_SCREEN		; SCREEN
	.byte	5,'P'
	.word	LBB_PIXEL		; PIXEL
	.byte	4,'L'
	.word	LBB_LINE		; LINE
	.byte	4,'O'
	.word	LBB_OVAL		; OVAL
...
ERR_NF	.by	'NEXT without FOR',$00
ERR_SN	.by	'Syntax',$00
ERR_RG	.by	'RETURN without GOSUB',$00
ERR_OD	.by	'Out of DATA',$00
ERR_FC	.by	'Function call',$00
ERR_OV	.by	'Overflow',$00
ERR_OM	.by	'Out of memory',$00
ERR_US	.by	'Undefined statement',$00
ERR_BS	.by	'Array bounds',$00
ERR_DD	.by	'Double dimension',$00
ERR_D0	.by	'Divide by zero',$00
ERR_ID	.by	'Illegal direct',$00
ERR_TM	.by	'Type mismatch',$00
ERR_LS	.by	'String too long',$00
ERR_ST	.by	'String too complex',$00
ERR_CN	.by	'Can' $27 't continue',$00
ERR_UF	.by	'Undefined function',$00
ERR_LD	.by	'LOOP without DO',$00
